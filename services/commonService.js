...
const { getRaceFromCache, getMatchFromCache, getAllSessionRedis, settingAllSessionMatchRedis, addMatchInCache, addRaceInCache, getExpertsRedisMatchData, getExpertsRedisSessionDataByKeys, getExpertsRedisKeyData, updateMultipleMarketSessionIdRedis, hasSessionInCache, getAllSessions } = require("./redis/commonfunction");
  ...
  if (userId) {
    if (isSessionAllowed) {
      ...
      if (redisIds?.length > 0) {
        let sessionData = await getAllSessions(matchId);
        match.sessionProfitLoss = sessionData[matchId] || {};
      }
    }
