...
const { getBlinkingTabs, getBlinkingTab, addBlinkingTabs, removeBlinkingTabs } = require("../services/blinkingTabsService");
const { getMatchById } = require("../services/matchService");
const { ErrorResponse, SuccessResponse } = require("../utils/response");

exports.addBlinkingTabsData = async (req, res) => {
    try {
        const { matchId, matchName, matchType } = req.body;
        ...
        await addBlinkingTabs({ createBy: userId, matchId: matchId, matchName: matchName, matchType: matchType });
        return SuccessResponse(
            {
                statusCode: 200,
                message: { msg: "created", keys: { name: "Match blinking tab" } }
            },
            req,
            res
        );  
